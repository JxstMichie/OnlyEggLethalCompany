    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private static bool Roll(ref SelectableLevel newLevel)
        {
            ...
            if (roll == 1)
            {
                ...
                if (Plugin.Rarity.Value < 0 || Plugin.Rarity.Value > 100)
                {
                    Plugin.Rarity.Value = 100;
                }

                // Temp list of items to add
                List<SpawnableItemWithRarity> itemsToAdd = new List<SpawnableItemWithRarity>();
                ...
                foreach (SpawnableItemWithRarity spawnableItemWithRarity in newLevel.spawnableScrap)
                {
                    ...
                    if (spawnableItemWithRarity.spawnableItem.itemName == Plugin.Item.Value)
                    {
                        spawnableItemWithRarity.rarity = Plugin.Rarity.Value;
                    }
                }

                // Check if the item needs to be added
                bool itemExists = newLevel.spawnableScrap.Any(s => s.spawnableItem.itemName == Plugin.Item.Value);
                if (!itemExists)
                {
                    foreach (Item item in StartOfRound.Instance.allItemsList.itemsList)
                    {
                        ...
                        {
                            itemsToAdd.Add(new SpawnableItemWithRarity
                            {
                                spawnableItem = item,
                ...
                // Now add the items to newLevel.spawnableScrap
                newLevel.spawnableScrap.AddRange(itemsToAdd);
            }
